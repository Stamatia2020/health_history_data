<!-- Φόρμα Μεταβολή/διαγραφή συγκεκριμένου ασθενή -->
...
<head>
	<meta charset="utf-8">
	<title>Φόρμα Μεταβολής/Διαγραφής Ασθενή</title>
...
{
// παιρνουμε τις τιμες των πεδιων απο το προγραμμα editselect_port.php
$empid=$_POST["empid"];

$sql = "SELECT empid,firstname,lastname,id, birthdate, sex, depid, cost, date_exam FROM employee where empid ='$empid'";
...
<hr size="3">
<br>
<h3 style="text-align: center;">Μεταβολή / Διαγραφή Ασθενή</h3>
<br>
<form name="form1" action="" method="post">
<table>
<tr><td class="al">Κωδικός Ασθενή:</td>
...
<tr><td class="al"><input type="button" value="Ακύρωση" onclick="document.location='editselect_port.php'">&nbsp;</td>
...
<?php
// αν έχει επιλεγεί το button submit-subok εκτελείται το if για μεταβολή Ασθενή
if (isset($_POST["subok"])) {  
  ...
  $sql = "UPDATE employee SET firstname='$fname', lastname='$lname', id='$id', birthdate='$birthdate', sex='$sex', depid='$depid', cost='$cost', date_exam='$date_exam' WHERE empid=$empid";

  if ($conn->query($sql)) {
     echo "<script>alert('Τα στοιχεία του ασθενή αποθηκεύτηκαν')</script>";
  ...
  echo "<script>document.location='editselect_port.php';</script>"; 
}
// αν έχει επιλεγεί το button submit-subdel εκτελείται το if για διαγραφή ασθενή
...
{ 
  ...
  if ($conn->query($sql)) {
    echo "<script>alert('Ο ασθενής διαγράφηκε')</script>";   
